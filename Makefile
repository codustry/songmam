...
check-safety:
	...
	$(BANDIT_COMMAND_FLAG)poetry run bandit -ll -r songmam/
...
check-style:
	...
	$(MYPY_COMMAND_FLAG)poetry run mypy --config-file setup.cfg songmam tests/**/*.py
...
.PHONY: test
test:
	poetry run pytest --ignore=tests/with_real_server

.PHONY: lint
lint: test check-safety check-style

# Example: make docker VERSION=latest
# Example: make docker IMAGE=some_name VERSION=1.7.2
.PHONY: docker
docker:
...
# Example: make clean_docker IMAGE=some_name VERSION=1.7.2
.PHONY: clean_docker
clean_docker:
